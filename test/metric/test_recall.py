...
from recnexteval.metrics import RecallK
...
class TestRecallK:
    ...
    def test_precision_k_values(self, recall_100_timestamp, recall_100, recall_default):
        assert recall_100_timestamp.K == 100
        assert recall_100.K == 100
        assert recall_default.K == 10

    def test_precision_identifier(self, recall_100_timestamp, recall_100, recall_default):
        assert recall_100_timestamp.identifier == "RecallK(timestamp_limit=123,K=100)"
    ...
    def test_precision_calculate(self, recall_default, X_pred, X_true):
        ...
        assert pytest.approx(recall_default.micro_result["score"], rel=0.01) == np.array([1, 0.67])
