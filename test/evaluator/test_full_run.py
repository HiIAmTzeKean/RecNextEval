import pytest
from recnexteval.datasets import TestDataset
from recnexteval.settings import SlidingWindowSetting, SingleTimePointSetting
from recnexteval.evaluators import EvaluatorPipelineBuilder, EvaluatorStreamerBuilder
...
def sliding_window():
    dataset = TestDataset()
    data = dataset.load()
    setting = SlidingWindowSetting(4, 3)
    setting.split(data)
    return setting
